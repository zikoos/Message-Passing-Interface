...
int main(int argc, char *argv[]) {
 /* rang du processus */
  ...
  if (h%p!=0)
    {
      fprintf(stderr, "error h/p n'est pas une division enti�re = %d\n", p);
    ...
    if (my_rank == MAITRE){
      ...
      /*debut chrono*/
      double debut_maitre = my_gettimeofday();
      ...
      for (i = 0; i < h_loc; i++) {
        ...
        /* r�ception des images locales des esclaves */
        int k=1;

        MPI_Probe (MPI_ANY_SOURCE, TAG_END, MPI_COMM_WORLD, &status);
        int s = status.MPI_SOURCE; // pour identidier l'emetteur
        if (s!=MAITRE){
          MPI_Recv(ima + s*w*h_loc * sizeof(unsigned char), h_loc*w, MPI_CHAR, s, TAG_END, MPI_COMM_WORLD, &status);
        }
      }
    ...
    if (my_rank != MAITRE){
      /* ymin_loc */
      ymin_loc=ymin + my_rank*yinc*h_loc;
      /* Allocation memoire du tableau resultat */
      pima=ima = (unsigned char *)malloc( w*h_loc*sizeof(unsigned char));
      if( ima == NULL) {
        fprintf( stderr, "Erreur allocation m�moire du tableau \n");
        return 0;
          }
      ...
      MPI_Send (ima, w*h_loc, MPI_CHAR, MAITRE, TAG_END, MPI_COMM_WORLD);
